        ...
        <>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-3 py-4 lg:py-24 gap-6 w-full lg:max-w-7xl relative '>
                {[
                    ...
                    <div key={index} className={`absolute z-10 lg:w-[30%] w-full shadow-xl   bg-white rounded-xl bottom-[-200px] p-6  hover:-translate-y-1 hover:scale-100 duration-300 ${index === 0 ? 'left-0' : index === 1 ? 'left-1/2 -translate-x-1/2' : 'right-0'}`}>
                        ...
                        <div className='flex items-center space-x-4'>
                            <Image src={profile.image} alt={`${profile.name} profile image`} width={50} height={50} className='rounded-full'/>
                            <div>
                                <p className='text-[#1f073b] font-faktum text-sm sm:text-base lg:text-lg font-bold'>{profile.name}</p>
                                <p className='text-[#1f073b] font-faktum text-xs sm:text-sm'>{profile.role}</p>
                            </div>
                        </div>
                    </div>
                ))}
