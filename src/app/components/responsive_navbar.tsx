...
export default function Nav({ isInView }: { isInView: boolean }) {
  ...
  const buttonClasses =
    "text-gray-100 font-bold text-sm px-2 py-1 border-2 border-gray-100 hover:bg-gray-100 hover:text-gray-800 rounded-lg trasition duration-300";
  const navClasses = `
...
`;
  ...
  const navStartNow =
    "bg-[#d93d8d] border border-[#d93d8d] font-faktum rounded-full font-bold items-center px-4 py-2  hover:text-underline";
  ...
  const buttons = (
    <>
      ...
      <a href="https://app.ameii.com.br/login">
      ...
      <a href="https://app.ameii.com.br/register">
    ...
    <>
      <div className="flex-row flex gap-4 justify-end">
        <Link
          ...
          className={`flex flex-row border border-1 px-6 gap-2  items-center font-faktum rounded-full p-2 ${
            isInView ? "text-white" : "text-[#1f073b] border-[#1f073b]"
          } hover:transform transition-transform duration-300 hover:scale-90 hover:bg-[#1f073b] hover:text-white`}
        >
        ...
        <Link
          ...
          className={`${navStartNow} hover:transform transition-transform duration-300 hover:scale-90 `}
        >
          <p>Comece já</p>
        </Link>
      </div>
  ...
  const navCenterLinks = (
    <>
      <div
        className={` hidden lg:flex  fixed left-1/2 top-[25px] -translate-x-1/2 gap-4 h-min `}
      >
        <Link
          href={""}
          className={`${
  ...
  return (
    <nav
      className={` text-white w-full sticky z-20 h-fit top-0 ${
        isInView ? "bg-[#1f073b]" : "bg-white"
      }`}
    >
